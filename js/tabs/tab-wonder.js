...
<!-- Header -->
<div class="hdr">
  <div class="hdr-brand">⚡ M.E.T. v3.7</div>
</div>
...
<!-- Footer -->
<div class="footer">
  <span>Generated by M.E.T. v3.7 — tools.brigelectric.com</span>
...
function wtCalc() {
  ...
  if (!candidates.length) {
    ...
    const altEntries = powerData.entries.filter(e => e[0] === altCores).sort((a,b) => a[1]-b[1]);
    const altCandidates = altEntries.filter(e => e[4] >= requiredPerRun);
    const altHint = altCandidates.length
      ? ` Try switching to <strong>${altCores}-core</strong> (largest available: ${altCandidates[0][1]}mm² rated ${altCandidates[0][4]} A).`
      : '';
  ...
  const minDerated = +(minCable[4] * combinedDerating).toFixed(1);
...
Generated by M.E.T. v3.7`;
  ...
  window._wtData = {
    ...
    isUpsized, prevCableCSA: prevCable?.[1], borderlineThreshold
  };
  ...
  const upsizeNote = isUpsized
    ? `<div class="wt-upsize-note">⬆ Upsized at engineer's discretion — minimum size was
        <strong>${prevCable[1]}mm²</strong> (${+(prevCable[4]*combinedDerating).toFixed(1)} A derated,
        ${(((showTotal?+(prevCable[4]*combinedDerating*parallel).toFixed(1):+(prevCable[4]*combinedDerating).toFixed(1))/fla-1)*100).toFixed(0)}% headroom — borderline).
        Selected <strong>${cCSA}mm²</strong> instead.
        <button class="btn" style="font-size:0.75rem;margin-left:10px" onclick="wtSetUpsized(false)">↩ Revert to ${prevCable[1]}mm²</button>
      </div>`
    : minIsBorderline && candidates.length > 1
      ? `<div class="wt-borderline-note">⚠ Selected cable is within the borderline margin (+${headroom}% headroom,
          ...
          <button class="btn primary" style="font-size:0.75rem;margin-left:10px" onclick="wtSetUpsized(true)">⬆ Use next size up (${candidates[1][1]}mm²)</button>
        </div>`
      : '';
