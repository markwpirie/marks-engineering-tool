...
<!-- Header -->
<div class="hdr">
  <div class="hdr-brand">⚡ M.E.T. v${APP_VERSION}</div>
</div>
...
<!-- Footer -->
<div class="footer">
  <span>Generated by M.E.T. v${APP_VERSION} — tools.brigelectric.com</span>
...
function wtCalc() {
  ...
  // ── Gland ──
  const glandResult = wtFindGland(glandPref, cOD, cOdTol);
...
Generated by M.E.T. v${APP_VERSION}`;
  ...
  result.innerHTML = `
    ...
    ${wtGlandCard(glandPref, cOD, useNPT, cOdTol)}
...
function wtFindGland(glandPref, OD, odTol) {
  let m;
  let type = glandPref;
  if (glandPref === '453') {
    m = pickRecommendedGland(findFittingGlands(GLAND_453, OD, odTol));
  } else if (glandPref === '653') {
    m = pickRecommendedGland(findFittingGlands(GLAND_653, OD, odTol));
  } else {
    type = '421';
...
function wtGlandCard(glandPref, OD, useNPT, odTol) {
  const codeTransform = useNPT ? wtNptCode : (c => c);
  if (glandPref === '453') {
    return renderGlandSizeList('453', findFittingGlands(GLAND_453, OD, odTol), useNPT, codeTransform);
  } else if (glandPref === '653') {
    return renderGlandSizeList('653', findFittingGlands(GLAND_653, OD, odTol), useNPT, codeTransform);
  } else {
